...
Clone the repository to your machine, then unzip the attached archives within your directory; these contain important template files. Note that these files can be fairly large (the largest, `allsne.pickle`, is over 150 MB). You will also need to set up your OS environment by setting the variable `DFDIR` to your installation directory, e.g. `export DFDIR=/home/mbaer/duperfit` with bash on my machine. One may add this to their `.bashrc` file if they so desire.
